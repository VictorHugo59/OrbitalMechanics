...
%we can use Astronomical Units to plot the field force over the orbit, just UNCOMMENT this section:
...
% t(1)=0;
% vx(1)=0;
% vy(1)=0.13; %Earth's velocity at perihelion
% x(1)=0.98330699034475; %Earth's distance at perihelion;
% y(1)=0.000000000000;
% radio(1)=0.98330699034475;
% vtot(1)=sqrt(vx(1).^2+vy(1).^2);
% for i=1:length(t)
%     x(2*i)=x(2*i-1)+vx(i).*0.5.*dt;
%     y(2*i)=y(2*i-1)+vy(i).*0.5.*dt;
...
% %we can create a sun for visualization
% %position is accurate, but size has been modified for better visualization
% rectangle('Position',[-0.1 -0.1 .2 .2],'Curvature',[1 1],'FaceColor',[1 1 0])
% 
